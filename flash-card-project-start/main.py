...
def next_card():
    ...
    word_choice = random.choice(to_learn)
    canvas.itemconfig(card_bg,image=card_front_img)
    canvas.itemconfig(card_title,text="French",fill="black")
    canvas.itemconfig(card_word,text=word_choice["French"],fill="black")
...
def flip_card():
    ...
    canvas.itemconfig(card_word,text=word_choice["English"],fill="white")

def is_known():
    to_learn.remove(word_choice)
    words_to_learn=pandas.DataFrame(to_learn)
    words_to_learn.to_csv("flash-card-project-start\\data\\words_to_learn.csv",index=False)
    next_card()
